...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($name === "" || $email === "" || $dob === "" || $gender === "" || $faculty === "" || $salary === "" || $phone === "") {
        ...
        if ($photoColumnExists) {
            $updateSql = "UPDATE employee SET name = '$name', email = '$email', dob = '$dob', gender = '$gender', faculty = '$faculty', salary = '$salary', phone = '$phone', address = '$address', status = 'Active', photo = '$photo'" . $passwordSqlPart . " WHERE id = $employeeId";
        } else {
            $updateSql = "UPDATE employee SET name = '$name', email = '$email', dob = '$dob', gender = '$gender', faculty = '$faculty', salary = '$salary', phone = '$phone', address = '$address', status = 'Active'" . $passwordSqlPart . " WHERE id = $employeeId";
        }

        if (mysqli_query($conn, $updateSql)) {
            $success = "Profile updated successfully.";

            $refreshSql = "SELECT * FROM employee WHERE id = $employeeId";
            $refreshRes = mysqli_query($conn, $refreshSql);
            $employee = mysqli_fetch_assoc($refreshRes);
        } else {
...
<div class="card">
    ...
    <form action="" method="post" enctype="multipart/form-data">
        ...
        <div class="inline-options">
            <input type="radio" name="gender" value="Male" id="pg_male" <?php if ($employee["gender"] === "Male") { echo "checked"; } ?>>
            <label for="pg_male">Male</label>
            <input type="radio" name="gender" value="Female" id="pg_female" <?php if ($employee["gender"] === "Female") { echo "checked"; } ?>>
            <label for="pg_female">Female</label>
            <input type="radio" name="gender" value="Other" id="pg_other" <?php if ($employee["gender"] === "Other") { echo "checked"; } ?>>
            <label for="pg_other">Other</label>
        </div>
